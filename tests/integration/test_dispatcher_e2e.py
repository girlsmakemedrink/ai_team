...
# === iter-9 Phase 3: MCPUnhealthyError → BLOCKED(mcp_unhealthy) ===


class _MCPUnhealthyBackend(BaseAgent):
    """Stub Backend whose handle() raises MCPUnhealthyError. iter-9
    dispatcher must catch this distinctly and emit a TASK_REPORT(
    status=blocked, blocked_on='mcp_unhealthy') — same posture as
    iter-6's LLMBudgetExhaustedError → BLOCKED, so dependents stay
    held in the HoldQueue (not cascade-dropped) and owner can fix
    the MCP misconfiguration and manually retry."""
    ...
    async def handle(self, msg: AgentMessage) -> list[AgentMessage]:
        ...
        raise MCPUnhealthyError(
            "MCP servers unhealthy (1): ai-team-repo: FileNotFoundError: "
...
async def test_mcp_unhealthy_emits_blocked_does_not_cascade_drop(
    redis_url: str,
    session_factory: async_sessionmaker[AsyncSession],
    db_session: AsyncSession,
) -> None:
    _ = db_session
    """When Backend raises MCPUnhealthyError, the synthesised report
    is BLOCKED with blocked_on='mcp_unhealthy' (not FAILED). QA
    (depends_on=[be]) stays held rather than getting dropped; root
    Task stays in_progress (no terminal cascade). Mirrors iter-6's
    LLMBudgetExhaustedError → BLOCKED contract. See iter_9.md
    success criterion #4 + iter_8_demo_report.md Failure 1."""
    bus = MessageBus.from_url(redis_url)
    feed = FeedPublisher.from_url(redis_url, db_session_factory=session_factory)
    signer = HMACSigner(_SECRET)
    audit = AuditLogWriter(session_factory, _SECRET)
    ...
    agents = {
        AgentId.TEAM_LEAD: TeamLeadAgent(llm=llm),
        AgentId.BACKEND_DEVELOPER: _MCPUnhealthyBackend(),
        AgentId.QA_ENGINEER: _StubQA(observer=qa_received),
    }
    task_state = TaskStateReducer(session_factory)
    dispatcher = AgentDispatcher(
    ...
    user_msg = signer.with_signature(
        AgentMessage(
            ...
            payload=TaskAssignmentPayload(
                task_id=root_task_id,
                title="iter-9 mcp-blocked path",
                description="Backend will raise MCPUnhealthyError.",
            ),
        )
    )
    ...
    try:
        await audit.write_message(user_msg, iteration=9)
        await bus.publish(user_msg)
        ...
        # Status is BLOCKED (not FAILED); blocked_on='mcp_unhealthy';
        # summary carries the MCPUnhealthyError name.
        be_payload = be_report_rows[0].payload_json.get("payload", {})
        assert be_payload.get("status") == TaskStatus.BLOCKED.value, (
            f"expected blocked, got {be_payload.get('status')!r}"
        )
        ...
        assert "MCPUnhealthyError" in be_payload.get("summary", "")

        # QA was NEVER delivered (held by HoldQueue under BLOCKED).
        await asyncio.sleep(1.0)
        assert qa_received == [], (
            f"QA should be held under BLOCKED(mcp_unhealthy), "
            f"but received {len(qa_received)} messages"
        )
