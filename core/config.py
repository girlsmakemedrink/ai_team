...
class Settings(BaseSettings):
    ...
    # === Auth (ADR-005) ===
    owner_token: SecretStr = Field(
        default=SecretStr("dev-token-replace-me-min-32-chars-aaaa"), min_length=32
    )
    hmac_secret: SecretStr = Field(
        default=SecretStr("dev-hmac-replace-me-min-32-chars-bbbbb"), min_length=32
    )
    ...
    # === Postgres ===
    postgres_dsn: str = "postgresql+asyncpg://ai_team:changeme-local-only@localhost:5432/ai_team"
