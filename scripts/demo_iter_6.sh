#!/usr/bin/env bash
# Iter-6 demo: same 6-stage DAG and direct-python MCP config as iter-5
# (PM → Architect → Backend | Designer → Frontend → QA per
# docs/sandbox/idea_validator_v2_spec.md). Three iter-6 fixes give the
# chain a real chance of reaching QA → pending_review this time:
#
#   1. Per-tier --max-budget-usd defaults raised (haiku $0.30,
#      sonnet $1.50, opus $4.00). iter-5 demo's Backend hit the
#      $0.50 sonnet cap after 13 turns; $1.50 lets a multi-file
#      implementation complete. See iter_5_demo_report.md Failure 1.
#   2. LLMBudgetExhaustedError → dispatcher BLOCKED. When `claude -p`
#      returns subtype=error_max_budget_usd, the dispatcher synthesises
#      TASK_REPORT(status=blocked, blocked_on='budget') instead of
#      failed — dependents stay held (not cascade-dropped) so the
#      owner can manually retry with elevated budget.
#   3. TaskStateReducer.on_drop terminalises dropped dependents. When
#      a predecessor's real FAILED cascade-drops a held dependent,
#      that dependent's child Task row now flips to failed too
#      (iter-5 left it in_progress indefinitely).
#
# Wall-clock bumped to 30 min (iter-5's 20 was too tight even on the
# green path; PM+Arch+Designer alone spent 7 min before Backend).
#
# Prerequisites:
...
RESP=$(curl -sf -X POST http://127.0.0.1:8000/api/tasks \
    ...
    -d '{
        "title": "iter-6 demo: idea-validator v2 (CLI + landing page + UX brief)",
