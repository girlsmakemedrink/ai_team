#!/usr/bin/env bash
# Iter-17 demo: same 6-stage DAG as iter-10..16 (PM → Architect
# → Backend | Designer → Frontend → QA per
# docs/sandbox/idea_validator_v2_spec.md). iter-17 fixes the
# 14-iteration LATENT MCP handshake bug at root: all three MCP
# servers now respond to claude -p's REQUIRED `initialize`
# JSON-RPC request. Pre-iter-17 the stdio loops silently dropped
# `initialize` → claude marked the server "still connecting" →
# ToolSearch retries → all the "still-connecting" / "never
# connected" / "unreachable" / "unavailable" symptoms iter-9..16
# routed correctly to BLOCKED but never fixed the underlying bug.
#
#   1. tools/mcp_servers/*/__main__.py now include an
#      `initialize` handler that responds with
#      protocolVersion + capabilities + serverInfo per MCP spec
#      (verified against
#      /modelcontextprotocol/modelcontextprotocol via context7).
#      `_build_response(msg)` is a pure function exposed for
#      unit testing — 12 unit tests + 6 integration subprocess
#      tests pin the handshake against regression.
#
#   2. The matcher/router work iter-10..16 still ships as
#      defense-in-depth. With iter-17's fix, Backend's MCP tools
#      should connect cleanly → pytest + git commit + push +
#      open PR succeed in one session → task_report(done) → QA
#      runs → pending_review → demo auto-approves. The chain
#      finally closes end-to-end for the first time in 17
#      iterations.
#
# Wall-clock budget: 30 min initial chain + 15 min retry window
# = 45 min total. Cost ceiling: $5.00 (iter-16 came in at $1.33
# with the matcher catching both Backend attempts; iter-17
# expected even lower if Backend completes on first try without
# any MCP race).
#
# Prerequisites:
...
MCP_CONFIG="$(pwd)/.iter17-mcp.json"
REPO_ROOT="$(pwd)"
...
RESP=$(curl -sf -X POST http://127.0.0.1:8000/api/tasks \
    ...
    -d '{
        "title": "iter-17 demo: idea-validator v2 (CLI + landing page + UX brief)",
...
step "6.5/7 — Auto-retry any BLOCKED Backend tasks (iter-17: defense-in-depth — should not fire if MCP fix works)"
    ...
    for r in data:
        ...
        subprocess.run(
            ["uv", "run", "ai-team", "approve", rid,
             "--comment", "iter-17 demo auto-approve"],
            check=False,
        )
PY
