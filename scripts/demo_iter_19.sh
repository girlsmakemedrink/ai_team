#!/usr/bin/env bash
# Iter-19 demo: same 6-stage DAG as iter-10..18 (PM → Architect
# → Backend | Designer → Frontend → QA per
# docs/sandbox/idea_validator_v2_spec.md). iter-19 closes the
# FIVE iter-18 demo caveats so a re-run produces a QA-emitted
# `pending_reviews` row with requesting_agent='qa_engineer'
# (not 'unknown', not PM).
#
#   1. iter-18 closed the formal owner-approval loop end-to-end
#      for the first time across 18 iterations — but the
#      historic-first row was written by PM (not QA, the
#      intended path) because PM's `allowed_tools = ()`
#      triggered claude -p's permissive default, and the row's
#      requesting_agent was 'unknown' because no agent role was
#      set per-invocation.
#
#   2. iter-19 closes all five iter-18 caveats:
#      - Phase 1: per-message env injection in BaseAgent +
#        custom PM/TL handle (AI_TEAM_AGENT_ROLE +
...
#      - Phase 3: explicit (Read, Glob, Grep) whitelist on PM
#        and TL. PM can no longer surprise-call MCP tools.
#      - Phase 4: PM llm_timeout_s 300 → 600 (joins the
#        LLM-bound majority).
#      - Phase 5: this script — poll filters on
#        requesting_agent=='qa_engineer' so PM (or any other
#        non-QA agent) writing a row doesn't break the loop
#        mid-chain; auto-approve uses ${REVIEWS_JSON:-[]}
#        belt-and-braces fallback against the iter-18 Caveat
#        4 JSONDecodeError.
#
#   3. Expected outcome: chain reaches QA done → QA calls
#      mcp__ai_team_tasks__request_human_review → row appears
#      in /api/reviews with requesting_agent='qa_engineer' →
#      demo auto-approve step succeeds → **formal
#      owner-approval loop runs end-to-end through the
#      INTENDED agent path for the first time**.
#
# Wall-clock budget: 30 min initial chain + 15 min retry window
# = 45 min total. Cost ceiling: $5.00 (iter-18 was $3.43 across
# two runs; iter-19 expects similar or lower since the iter-18
# Backend tree is already on disk).
#
# Prerequisites:
...
MCP_CONFIG="$(pwd)/.iter19-mcp.json"
REPO_ROOT="$(pwd)"
...
RESP=$(curl -sf -X POST http://127.0.0.1:8000/api/tasks \
    ...
    -d '{
        "title": "iter-19 demo: idea-validator v2 (CLI + landing page + UX brief)",
    ...
    for r in data:
        ...
        subprocess.run(
            ["uv", "run", "ai-team", "approve", rid,
             "--comment", "iter-19 demo auto-approve"],
            check=False,
        )
PY
