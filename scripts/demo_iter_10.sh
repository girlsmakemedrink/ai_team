#!/usr/bin/env bash
# Iter-10 demo: same 6-stage DAG and direct-python MCP config as
# iter-9 (PM → Architect → Backend | Designer → Frontend → QA per
# docs/sandbox/idea_validator_v2_spec.md). Two iter-10 fixes
# target the narrow mid-session failure mode iter-9 demo surfaced:
#
#   1. Dispatcher substring router on task_report(failed) summaries.
#      iter-9's pre-flight gate catches DETERMINISTIC startup
#      failures (module imports, env validation) but the iter-8 +
#      iter-9 demo Backend failures were MID-SESSION (claude -p's
#      MCP subprocess spawn race AFTER our in-process probe
#      passes). The LLM detects this and emits a schema-valid
#      task_report(failed) whose summary names the failure
#      verbatim ("MCP server * never connected" / "* never
#      finished connecting" / "* still connecting"). iter-10's
#      substring router catches these patterns in the dispatcher's
#      outbound loop and rewrites status=failed → status=blocked,
#      blocked_on='mcp_unhealthy' before HMAC-sign. Dependents
#      stay held in HoldQueue (mirrors iter-6 + iter-9 BLOCKED
#      contracts). See iter_9_demo_report.md Failure 1.
#
#   2. Backend system prompt: forbid native Bash for git/uv/make/
#      pytest commands. iter-9 demo Backend admitted the Bash
#      tool was blocking its git commits even though Bash isn't
#      in its allowed_tools — the LLM was perceiving a gap and
#      reaching for it instead of using mcp__ai_team_repo__run_shell
#      with the appropriate command_class. iter-10 prompt update
#      adds a lookup table of "what command_class to use for each
#      common operation" so the LLM can't perceive a gap.
#
# Wall-clock stays at 30 min (iter-6's 30 is still the right shape).
#
# Prerequisites:
...
MCP_CONFIG="$(pwd)/.iter10-mcp.json"
REPO_ROOT="$(pwd)"
...
RESP=$(curl -sf -X POST http://127.0.0.1:8000/api/tasks \
    ...
    -d '{
        "title": "iter-10 demo: idea-validator v2 (CLI + landing page + UX brief)",
