#!/usr/bin/env bash
# Iter-8 demo: same 6-stage DAG and direct-python MCP config as iter-7
# (PM → Architect → Backend | Designer → Frontend → QA per
# docs/sandbox/idea_validator_v2_spec.md). Three iter-8 fixes target
# the two narrow failure modes iter-7 demo surfaced:
#
#   1. Designer's per-call llm_timeout_s raised to 600s (matches
#      Architect / Backend / Frontend / DevOps). iter-7 demo's
#      Designer timed out at the 300s BaseAgent default on the v2
#      UX brief + landing-page wireframe task. See
#      iter_7_demo_report.md Failure 1.
#   2. _is_budget_exhausted_stdout substring-match. iter-6's BLOCKED
#      detector required a complete json.loads() parse, but the
#      adapter capped stdout at 2 KB — real Backend error JSONs
#      get truncated mid-object, the parse failed, and exhaustion
#      routed to FAILED + cascade-drop instead of BLOCKED + owner
#      manual retry. Substring-only detector + stdout cap raised
#      2 KB → 8 KB. See iter_7_demo_report.md Failure 2.
#   3. Sonnet --max-budget-usd raised $1.50 → $2.50. iter-7 demo
#      Backend hit $1.50 at 11 turns; $2.50 gives ~18 turns of
#      headroom for a full v2 implementation + commit + open PR.
#      See iter_7_demo_report.md Failure 3.
#
...
RESP=$(curl -sf -X POST http://127.0.0.1:8000/api/tasks \
    ...
    -d '{
        "title": "iter-8 demo: idea-validator v2 (CLI + landing page + UX brief)",
