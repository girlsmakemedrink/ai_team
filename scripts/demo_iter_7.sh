#!/usr/bin/env bash
# Iter-7 demo: same 6-stage DAG and direct-python MCP config as iter-6
# (PM → Architect → Backend | Designer → Frontend → QA per
# docs/sandbox/idea_validator_v2_spec.md). Three iter-7 fixes give the
# chain a real chance of finally reaching QA → pending_review:
#
#   1. Architect's per-call llm_timeout_s raised to 600s (matches
#      Backend / Frontend / DevOps). iter-6 demo's Architect timed out
#      at the 300s BaseAgent default on the v2 ADR + system-design
#      draft. See iter_6_demo_report.md Failure 1.
#   2. LLMTimeoutError now carries buffered stdout (best-effort drain
#      after kill). iter-6 demo's Architect timeout was a diagnostic
#      dead-end; iter-5 Phase 4 parity for the timeout branch.
#   3. Dispatcher cascades drops through HoldQueue. When a
#      predecessor's FAILED drops a dependent, the dispatcher walks
#      the dropped task_ids back through HoldQueue.mark_failed so
#      further-downstream dependents (fe → qa in arch→design→fe→qa)
#      also drop. iter-6 left fe + qa stuck in_progress after design
#      was dropped. See iter_6_demo_report.md Failure 2.
#
# Wall-clock stays at 30 min (iter-6's 30 was the right shape; v2-
# shaped full run is bounded by individual agent timeouts now that
# Architect has 600s headroom).
#
# Prerequisites:
...
RESP=$(curl -sf -X POST http://127.0.0.1:8000/api/tasks \
    ...
    -d '{
        "title": "iter-7 demo: idea-validator v2 (CLI + landing page + UX brief)",
