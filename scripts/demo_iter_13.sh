#!/usr/bin/env bash
# Iter-13 demo: same 6-stage DAG as iter-10..12 (PM → Architect
# → Backend | Designer → Frontend → QA per
# docs/sandbox/idea_validator_v2_spec.md). One iter-13 fix
# closes the claude -p session-id durability gap iter-12 demo
# surfaced + this script now exercises retry-blocked
# end-to-end and auto-approves QA's pending_review.
#
#   1. ClaudeCodeHeadlessClient now catches "Session ID is
#      already in use" stderr and retries once with --resume.
#      iter-12 demo hit this when uvicorn was killed between
#      Backend's BLOCKED report and the retry-blocked call.
#      Now restart-resilient (proven by unit tests); production
#      dispatchers staying alive across BLOCKED→retry use the
#      cache path with no extra spawn.
#
...
#      for QA's pending_review, then auto-approves. If the
#      chain closes end-to-end (Backend retry → DONE → QA →
#      pending_review → approved), iter-13 closes the
#      pending_review loop iter-3..12 all reached for.
#
# Wall-clock budget: 30 min initial chain + 15 min retry window
# = 45 min total. Cost ceiling: $5.00 (iter-12 came in at
# $1.32; iter-13 adds a successful Backend retry's spend).
#
# Prerequisites:
...
MCP_CONFIG="$(pwd)/.iter13-mcp.json"
REPO_ROOT="$(pwd)"
...
RESP=$(curl -sf -X POST http://127.0.0.1:8000/api/tasks \
    ...
    -d '{
        "title": "iter-13 demo: idea-validator v2 (CLI + landing page + UX brief)",
    ...
    for r in data:
        ...
        subprocess.run(
            ["uv", "run", "ai-team", "approve", rid,
             "--comment", "iter-13 demo auto-approve"],
            check=False,
        )
PY
