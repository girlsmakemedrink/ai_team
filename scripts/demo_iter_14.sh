#!/usr/bin/env bash
# Iter-14 demo: same 6-stage DAG as iter-10..13 (PM → Architect
# → Backend | Designer → Frontend → QA per
# docs/sandbox/idea_validator_v2_spec.md). One iter-14 fix —
# one additional pattern tuple in the substring router —
# catches iter-13 demo's THIRD distinct Backend MCP-race
# phrasing so the chain can finally close end-to-end.
#
#   1. _MCP_RACE_PATTERNS now includes ("mcp__ai_team_repo",
#      "never connected"). iter-13 demo's Backend retry session
#      (correlation 1e7bb0db-...) used this exact wording and
#      escaped both iter-10's three tuples (require "MCP server")
#      and iter-12's two tuples (require "unavailable").
#      With iter-14 the router routes that wording to
#      BLOCKED(mcp_unhealthy) instead of FAILED, keeping QA
#      held + retry path live.
#
#   2. This script's tail (step 6.5/7) auto-detects BLOCKED
#      Backend, runs `ai-team retry-blocked <task_id>` WITHIN
#      the same dispatcher process, waits up to 15 more min
#      for QA's pending_review, then auto-approves. With
#      iter-13's session-id fallback + iter-14's new tuple +
#      Backend's implementation tree on disk from iter-13's
#      --resume session, the chain should finally close
#      end-to-end (Backend → DONE → QA → pending_review →
#      approved). The long-awaited iter-3..13 close.
#
# Wall-clock budget: 30 min initial chain + 15 min retry window
# = 45 min total. Cost ceiling: $5.00 (iter-13 came in at
# $1.86; iter-14 expected lower with the cache hot + tree on
# disk).
#
# Prerequisites:
...
MCP_CONFIG="$(pwd)/.iter14-mcp.json"
REPO_ROOT="$(pwd)"
...
RESP=$(curl -sf -X POST http://127.0.0.1:8000/api/tasks \
    ...
    -d '{
        "title": "iter-14 demo: idea-validator v2 (CLI + landing page + UX brief)",
...
step "6.5/7 — Auto-retry any BLOCKED Backend tasks (iter-14: close the loop)"
    ...
    for r in data:
        ...
        subprocess.run(
            ["uv", "run", "ai-team", "approve", rid,
             "--comment", "iter-14 demo auto-approve"],
            check=False,
        )
PY
