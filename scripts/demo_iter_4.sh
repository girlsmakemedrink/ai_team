#!/usr/bin/env bash
# Iter-4 demo: same 6-stage DAG as iter-3 (PM → Architect → Backend |
# Designer → Frontend → QA per docs/sandbox/idea_validator_v2_spec.md)
# but MCP servers are spawned via `${REPO_ROOT}/.venv/bin/python -m …`
# instead of `uv run python -m …`. That removes ~15 ms + variance from
# each MCP cold-start (see docs/iterations/iter_4_mcp_benchmark.md and
# iter_3_demo_report.md Failure 2). Wall-clock stays at 20 min.
#
# Also: TL now emits a DAG-preview BROADCAST (iter-4 Phase 4) which the
# owner sees in `ai-team watch` seconds before agents start.
#
# Prerequisites:
...
RESP=$(curl -sf -X POST http://127.0.0.1:8000/api/tasks \
    ...
    -d '{
        "title": "iter-4 demo: idea-validator v2 (CLI + landing page + UX brief)",
