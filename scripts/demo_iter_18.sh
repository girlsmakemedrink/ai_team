#!/usr/bin/env bash
# Iter-18 demo: same 6-stage DAG as iter-10..17 (PM → Architect
# → Backend | Designer → Frontend → QA per
# docs/sandbox/idea_validator_v2_spec.md). iter-18 closes the
# FORMAL OWNER-APPROVAL LOOP by implementing the iter-0
# stub-replaced `mcp__ai_team_tasks__request_human_review` MCP
# tool — it now INSERTs a real `pending_reviews` row that the
# /api/reviews endpoint surfaces and the auto-approve step
# resolves via POST /api/reviews/{id}/approve.
#
#   1. iter-17 fixed the 14-iteration latent MCP handshake bug
#      (initialize handler) + the iter-5 permission-mode gap
#      (bypassPermissions). Run #3 produced the first end-to-end
#      7-agent chain completion in project history, but QA's
#      task_report(done) didn't materialise a pending_review row
#      because the MCP tool was still the iter-0 stub.
#
#   2. iter-18 replaces the stub with a real handler:
#      tools/mcp_servers/ai_team_tasks/handlers.py — Context +
#      handle_request_human_review (INSERT PendingReview row) +
#      stubs preserved for mark_task_done / update_task_status
#      pending an agent-prompt audit. The QA prompt now
#      instructs an explicit request_human_review call before
#      the final JSON output (correlation_id copied verbatim
#      from the message header).
#
#   3. Expected outcome: chain reaches QA done → QA calls the
#      MCP tool → row appears in /api/reviews → demo
#      auto-approve step succeeds → **formal owner-approval
#      loop closes end-to-end for the first time across 18
#      iterations**.
#
# Wall-clock budget: 30 min initial chain + 15 min retry window
# = 45 min total. Cost ceiling: $5.00 (iter-17 run #3 was $5.69
# with full Backend reimplementation; iter-18 expects lower
# since Backend's idea-validator tree is already on disk from
# iter-17 — Backend should find existing code and recap).
#
# Prerequisites:
...
MCP_CONFIG="$(pwd)/.iter18-mcp.json"
REPO_ROOT="$(pwd)"
...
RESP=$(curl -sf -X POST http://127.0.0.1:8000/api/tasks \
    ...
    -d '{
        "title": "iter-18 demo: idea-validator v2 (CLI + landing page + UX brief)",
    ...
    for r in data:
        ...
        subprocess.run(
            ["uv", "run", "ai-team", "approve", rid,
             "--comment", "iter-18 demo auto-approve"],
            check=False,
        )
PY
