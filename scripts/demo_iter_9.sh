#!/usr/bin/env bash
# Iter-9 demo: same 6-stage DAG and direct-python MCP config as iter-8
# (PM → Architect → Backend | Designer → Frontend → QA per
# docs/sandbox/idea_validator_v2_spec.md). One iter-9 fix targets
# the narrow new failure mode iter-8 demo surfaced:
#
#   Pre-flight MCP health-gate in BaseAgent.handle(). Probes each
#   declared tools.mcp_servers.* module (importlib.import_module +
#   Context.from_env for ai_team_repo) BEFORE claude -p spawns the
#   MCP subprocess. Catches deterministic startup failures —
#   ModuleNotFoundError, AI_TEAM_REPO_ROOT misconfiguration — that
#   iter-8 demo's Backend hit as opaque "still connecting" timeouts
#   inside its claude -p session. Failures now route to
#   BLOCKED(mcp_unhealthy) + owner manual retry (dependents stay
#   held in HoldQueue) instead of FAILED + cascade-drop. See
...
RESP=$(curl -sf -X POST http://127.0.0.1:8000/api/tasks \
    ...
    -d '{
        "title": "iter-9 demo: idea-validator v2 (CLI + landing page + UX brief)",
