#!/usr/bin/env bash
# Iter-5 demo: same 6-stage DAG and direct-python MCP config as iter-4
# (PM → Architect → Backend | Designer → Frontend → QA per
# docs/sandbox/idea_validator_v2_spec.md). Three iter-5 fixes give the
# chain a real chance of running to completion this time:
#
#   1. Dispatcher synthesises TASK_REPORT(failed) when handle() raises,
#      so an agent crash no longer hangs HoldQueue dependents (iter-4
#      Backend exit-1).
#   2. `claude -p` agent sessions pass --permission-mode acceptEdits, so
#      Frontend's file writes no longer stall on an interactive
#      permissions prompt (iter-4 Failure 2).
#   3. Every agent stamps metadata['llm'] on its outputs, so the
#      single-SQL demo report shows real metrics for every row
#      (iter-3 + iter-4 reports had gaps for 5+ agents).
#
# Wall-clock stays at 20 min.
#
# Prerequisites:
...
RESP=$(curl -sf -X POST http://127.0.0.1:8000/api/tasks \
    ...
    -d '{
        "title": "iter-5 demo: idea-validator v2 (CLI + landing page + UX brief)",
