#!/usr/bin/env bash
# Iter-11 demo: same 6-stage DAG as iter-10 (PM → Architect →
# Backend | Designer → Frontend → QA per
# docs/sandbox/idea_validator_v2_spec.md). Three iter-11 fixes
# build on iter-10's recoverable BLOCKED state:
#
#   1. Retry mechanism. The owner can recover from a BLOCKED
#      Backend task with:
#        uv run ai-team retry-blocked <task_id>
#      The endpoint reads the audit_log for the task, validates
#      the BLOCKED status + recoverable blocked_on, then re-emits
#      the original task_assignment with the same task_id +
#      correlation_id (HoldQueue dependents key off task_id) but
#      a fresh message_id and bumped metadata.retry_attempt.
#      Capped at 5 attempts total.
#
#   2. Backend Bash defense-in-depth. iter-10 prompt edit added a
#      command_class lookup table, but the LLM still reached for
#      Bash and reported "Bash hooks blocked the pytest command".
#      iter-11 belt-and-suspenders: BackendDeveloperAgent now
#      declares disallowed_tools=("Bash",), which translates to
#      claude -p's --disallowed-tools flag. Bash is refused before
#      the LLM can attempt it.
#
#   3. BaseAgent.llm_timeout_s default flipped 300→600. Zero
#      behavior change (5 agents were already overriding to 600);
#      anchors the default at the LLM-bound majority case.
#
# Wall-clock stays at 30 min. When Backend lands BLOCKED, this
# script does NOT automatically retry — leaving the owner action
# explicit matches ADR-001's posture. The script prints the
# retry-blocked invocation for the owner to copy/paste.
#
# Prerequisites:
...
MCP_CONFIG="$(pwd)/.iter11-mcp.json"
REPO_ROOT="$(pwd)"
...
RESP=$(curl -sf -X POST http://127.0.0.1:8000/api/tasks \
    ...
    -d '{
        "title": "iter-11 demo: idea-validator v2 (CLI + landing page + UX brief)",
