#!/usr/bin/env bash
# Iter-15 demo: same 6-stage DAG as iter-10..14 (PM → Architect
# → Backend | Designer → Frontend → QA per
# docs/sandbox/idea_validator_v2_spec.md). Two iter-15 fixes —
# a structural shift in the MCP-race matcher + 429 routing —
# should let the chain finally close end-to-end after five
# iterations of one-tuple-per-iteration with diminishing
# returns (iter-14 demo report's "Failure 1").
#
#   1. Cross-product MCP-race matcher: `_MCP_RACE_PATTERNS`
#      (tuple-of-tuples) is replaced by `_MCP_TOKEN_SET` x
#      `_MCP_FAILURE_VERB_SET` — any MCP-naming token co-
#      occurring with any failure verb routes to BLOCKED.
#      Covers all four previously-observed phrasings (iter-9,
#      iter-11, iter-13, iter-14) + their natural variations
#      via a 3 x 7 = 21-combination cross product.
#
#   2. api_error_status=429 in claude -p stdout now raises
#      LLMBudgetExhaustedError → dispatcher emits
#      BLOCKED(blocked_on='budget'). iter-14 run #1 burned
#      $0.59 on Architect's Max-5x session-limit truncation;
#      with iter-15 that becomes a BLOCKED row recoverable
#      via retry-blocked once the reset window opens.
#
# Combined with iter-13's session-id collision fallback + the
# implementation tree on disk from iter-13's --resume session,
# the chain should finally close end-to-end (Backend → DONE →
# QA → pending_review → approved). The long-awaited iter-3..14
# close.
#
# Wall-clock budget: 30 min initial chain + 15 min retry window
# = 45 min total. Cost ceiling: $5.00 (iter-14 came in at
# $2.48 total across two runs; iter-15 expected lower if 429-
# routing engages cleanly and the cross-product matcher catches
# any race that fires).
#
# Prerequisites:
...
MCP_CONFIG="$(pwd)/.iter15-mcp.json"
REPO_ROOT="$(pwd)"
...
RESP=$(curl -sf -X POST http://127.0.0.1:8000/api/tasks \
    ...
    -d '{
        "title": "iter-15 demo: idea-validator v2 (CLI + landing page + UX brief)",
...
step "6.5/7 — Auto-retry any BLOCKED Backend tasks (iter-15: close the loop)"
    ...
    for r in data:
        ...
        subprocess.run(
            ["uv", "run", "ai-team", "approve", rid,
             "--comment", "iter-15 demo auto-approve"],
            check=False,
        )
PY
